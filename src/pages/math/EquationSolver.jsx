                ...
                <div className="bg-white/50 backdrop-blur-sm rounded-3xl p-6 shadow-sm flex flex-col items-center min-h-[160px] justify-center">
                    ...
                    {step === 0 && (
                        <div className="flex flex-col items-center gap-4">
                            <div className="flex items-center gap-2 sm:gap-4 font-bold font-serif text-2xl sm:text-3xl md:text-4xl">
                                {equationType === 'unknown_plus_a' && (
                                    <>
                                        <span className="text-violet-600">?</span>
                                        <span className="text-sky-500 mx-1">+</span>
                                        <div className="flex flex-col items-center bg-sky-50 rounded-xl px-3 py-1 border-2 border-sky-200">
                                            <button
                                                onClick={() => leftCount < 20 && (setLeftCount(leftCount + 1), totalCount <= leftCount && setTotalCount(leftCount + 1), resetScene())}
                                                disabled={leftCount >= 20}
                                                className="text-sky-500 text-sm sm:text-base font-bold touch-manipulation active:scale-90 disabled:text-slate-300 disabled:cursor-not-allowed px-2"
                                            >▲</button>
                                            <span className="text-sky-600 min-w-[2rem] text-center py-1">{leftCount}</span>
                                            <button
                                                onClick={() => leftCount > 1 && (setLeftCount(leftCount - 1), resetScene())}
                                                disabled={leftCount <= 1}
                                                className="text-sky-500 text-sm sm:text-base font-bold touch-manipulation active:scale-90 disabled:text-slate-300 disabled:cursor-not-allowed px-2"
                                            >▼</button>
                                        </div>
                                        <span className="mx-1">=</span>
                                        <div className="flex flex-col items-center bg-slate-50 rounded-xl px-3 py-1 border-2 border-slate-200">
                                            <button
                                                onClick={() => totalCount < 30 && (setTotalCount(totalCount + 1), resetScene())}
                                                disabled={totalCount >= 30}
                                                className="text-slate-500 text-sm sm:text-base font-bold touch-manipulation active:scale-90 disabled:text-slate-300 disabled:cursor-not-allowed px-2"
                                            >▲</button>
                                            <span className="text-slate-700 min-w-[2rem] text-center py-1">{totalCount}</span>
                                            <button
                                                onClick={() => totalCount > leftCount && (setTotalCount(totalCount - 1), resetScene())}
                                ...
                                {equationType === 'total_minus_unknown' && (
                                    <>
                                        ...
                                        <span className="mx-1" style={{ color: '#FF7F50' }}>−</span>
                                        <span className="text-violet-600">?</span>
                                        <span className="mx-1">=</span>
                                        <div className="flex flex-col items-center bg-sky-50 rounded-xl px-3 py-1 border-2 border-sky-200">
                                            <button
                                                onClick={() => leftCount < totalCount && (setLeftCount(leftCount + 1), resetScene())}
                                                disabled={leftCount >= totalCount}
                                                className="text-sky-500 text-sm sm:text-base font-bold touch-manipulation active:scale-90 disabled:text-slate-300 disabled:cursor-not-allowed px-2"
                                            >▲</button>
                                            <span className="text-sky-600 min-w-[2rem] text-center py-1">{leftCount}</span>
                                            <button
